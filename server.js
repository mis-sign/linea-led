...
// ==========================================
// WHATSAPP GATEWAY — Maytapi Integration
// ==========================================
...
async function sendWhatsApp(number, message) {
    ...
    if (provider === 'mock') {
        console.log(`[MOCK WhatsApp -> ${number}]: ${message}`);
    ...
    if (provider === 'maytapi') {
        try {
            ...
            if (!productId || !phoneId || !token) {
                console.warn('[Maytapi] Missing env vars: PRODUCT_ID, META_WA_PHONE_ID or META_WA_TOKEN');
            ...
            console.log(`[Maytapi -> ${toNumber}] ${resp.ok ? '✅ Sent' : '❌ Failed'}:`, JSON.stringify(data));
            return { success: resp.ok, data };

        } catch (err) {
            console.error('[Maytapi] Error:', err.message);
            return { success: false, error: err.message };
        }
    }

    // Meta Cloud API (kept as fallback)
    if (provider === 'meta') {
        try {
            const resp = await fetch(`https://graph.facebook.com/v20.0/${process.env.META_WA_PHONE_ID}/messages`, {
                ...
                body: JSON.stringify({ messaging_product: 'whatsapp', to: formatPhoneNumber(number), type: 'text', text: { body: message } })
            });
...
app.patch('/api/admin/complaints/:id/status', async (req, res) => {
    // Note: No admin key required for status update — field executives use the dashboard directly
